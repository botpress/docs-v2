...
When the expire timer fires, the framework runs this sequence:

1. **Race guard check**: did the user send a message after this expiration was scheduled? If yes, skip silently — timers are already rescheduled.
2. **Your handler runs** with `type: "expire"`.
3. **Framework cleanup** runs in order:
   1. Cancel active workflows
   2. Cancel pending nudge
   3. Set `session.status` to `expired`
   4. Tag the conversation
   5. Reset user state
   6. Clear the LLM transcript
4. **Done.** The next message triggers session renewal.
