...
import evalsConsoleImg from '../assets/evals-console.png'
import evalsConsoleDarkImg from '../assets/evals-console-dark.png'
  ...
  conversation: [
    {
      user: 'Hello',
      assert: {
        response: [{ llm_judge: 'The response is a friendly greeting' }],
      },
    },
  ],
})
...
new Eval({
  name: 'full-flow',
  conversation: [{ user: 'Create a ticket for VPN issues' }, { user: 'Set priority to high' }],
  outcome: {
    tables: [{ table: 'TicketTable', row_exists: { priority: { equals: 'high' } } }],
    workflow: [{ name: 'createTicket', completed: true }],
  },
})
```
...
<Frame>
  <Picture alt="Evals page in dev console" class="block dark:hidden" src={evalsConsoleImg} />
  <Picture alt="Evals page in dev console" class="hidden dark:block" src={evalsConsoleDarkImg} />
</Frame>
