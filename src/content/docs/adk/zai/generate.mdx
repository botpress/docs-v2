...
const intro = await adk.zai.text('Write a brief introduction about AI in customer service', { length: 200 })
```
...
```typescript
const formal = await adk.zai.rewrite('hey, the meeting is tmrw at 3', 'Make this professional and formal')
...
const patched = await adk.zai.patch(files, 'Add JSDoc comments to all exported functions')
```
