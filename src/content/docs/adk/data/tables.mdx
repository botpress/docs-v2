...
import tablesConsoleImg from '../assets/tables-console.png'
import tablesConsoleDarkImg from '../assets/tables-console-dark.png'
...
<Frame>
  <Picture alt="Tables page in dev console" class="block dark:hidden" src={tablesConsoleImg} />
  <Picture alt="Tables page in dev console" class="hidden dark:block" src={tablesConsoleDarkImg} />
</Frame>
...
<Note>If `keyColumn` is omitted on both the table defintion and the `upsertRows` call, it defaults to `id`.</Note>

### Delete rows

<CodeGroup>

```typescript title="Delete rows by name"
await OrderTable.deleteRows({ status: 'cancelled' })
```

```typescript title="Delete rows by ID"
await OrderTable.deleteRowIds([1, 2, 3])
```

```typescript title="Delete all rows"
...
const { rows } = await OrderTable.findRows({
  filter: {
    $or: [{ status: 'pending' }, { total: { $gt: 500 } }],
  },
})
```
