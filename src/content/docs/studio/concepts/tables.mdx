...
```javascript
const { table, indexing, rows, stale } = await client.getTable({
  table: 'Data1Table',
})
```
...
<Picture src={tableCodeOperationsImg} alt="" />
...
<Picture src={computedColumnSwitchImg} alt="" />
...
<Picture src={tableDependenciesImg} alt="" />
...
<Picture src={editCodeButtonImg} alt="" />

You will see a window with a code editor.

<Picture src={codeEditorImg} alt="" />
