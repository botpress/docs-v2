...
import { Note, Warning } from '@/components/Callout.tsx'
...
<Note>This method is asynchronous. Make sure to `await` it or handle the promise.</Note>
...
<Warning>
  Plugins must be registered before calling `app.start()`. Registering plugins after the server has started will throw
  an error.
</Warning>
