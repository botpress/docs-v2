...
<Note>
  This example uses [`dotenv` ](https://www.npmjs.com/package/dotenv) to manage Botpress credentials. If you'd rather
...
```ts title="Using the Botpress client" {10-14}
import { Client } from '@botpress/client'
...
```ts title="Using an HTTP request" {17-21}
