...
In WhatsApp, choices require a label. Since Botpress Studio doesn't currently support labels for choice Cards, WhatsApp uses _“Choose…”_ by default.
...
<Note>
  You can also set a custom file name by sending the file using an [Execute Code
  Card](/studio/concepts/cards/execute-code) and specifying a value for the `filename` attribute.
</Note>
