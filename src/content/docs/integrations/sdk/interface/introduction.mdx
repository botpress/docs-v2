...
<Warning>
  Interface creation is **currently restricted to the Botpress Team**. Support for developer-created interfaces _may_ be
  introduced in future releases at Botpress's sole discretion.
</Warning>
...
<Tip>
  Integrations can rename an interface's events, actions, channels, and entities to better align with the concepts and
  terminology of their specific external service. Integrations can also extend entities with additional fields to
  accommodate the requirements of their external service.
</Tip>
