...
import getUserDataImg from './assets/get-user-data.png'
import getUserDataDarkImg from './assets/get-user-data-dark.png'
...
<Warning>
  The `updateUser` method is only available after Webchat has been initialized. To avoid errors, consider [listening for
  the initialization event](/webchat/interact/listen-to-events#webchat-is-initialized) before calling `updateUser`.
</Warning>
...
<Frame>
  <Picture alt="Get User Data Card" class="block dark:hidden" src={getUserDataImg} />
  <Picture alt="Get User Data Card" class="hidden dark:block" src={getUserDataDarkImg} />
</Frame>
