  ...
  for (const item of pages) {
    if (typeof item === 'string') {
      ...
      if (item.root) {
        ...
        const firstChildHref = findFirstHref(buildPages(item.pages, 0, '', titleMap, new Map(), sidebarTitleMap))
        groupPrefix.push({ label: item.group, href: firstChildHref ?? '/' })
      }

      const result = await searchPagesForBreadcrumbs(pagePath, item.pages, groupPrefix, titleMap, sidebarTitleMap)
...
export async function buildBreadcrumbs<TCollection extends string>(
  ...
  for (const tab of config.navigation.tabs) {
    const crumbs = await searchPagesForBreadcrumbs(pagePath, tab.pages, [], titleMap, sidebarTitleMap)
    if (crumbs) {
